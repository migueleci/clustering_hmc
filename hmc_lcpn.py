...
for j, root in enumerate(root_list):
  ...
  lcpn = open("{0}/lcpn.txt".format(path), 'r') # load lcpn order
  lcpn = [x.strip() for x in lcpn.readlines()]
  ...
  """
  k-fold, the lcpn approach is applied for each fold independently and the mean
  of the performance for each fold is the result
  """
  N = 10
  feature_importance_ = [None for _ in lcpn]
  columns_ = [None for _ in lcpn]
  ...
  for train_index, test_index in tqdm(kfold.split(data), total=N, ascii="-#", desc="SHAP"): # set the same folds for each model
    ...
    count = 0
    for line in lcpn:
      ancestor, children = line.strip().split('=') # level info parent_list= children_list
      ancestor = ancestor.strip()
      children = np.array(children.strip().split(','))
      ...
      """
      cols = [c for c in data.columns if "GO:"+c[4:] in children or "GO:"+c[4:] == ancestor]
      ...
      """
      # random forest
      estimator.fit(X_train, y_train if len(children) > 1 else y_train.values.ravel())
      ...
      if y_train.shape[1] == 1:
        ...
        assert y.shape[1]*2 == shap_values.shape[0] or y.shape[1] == shap_values.shape[0]
        for fidx in range(y.shape[1]):
          if y.shape[1] == shap_values.shape[0]:
            vals = abs(shap_values[fidx]).mean(0)
          elif y.shape[1]*2 == shap_values.shape[0]:
  ...
  for level in range(len(lcpn)):
    ...
    fimp.to_csv('{0}/features_lcpn_{1}.csv'.format(path,level), index=False)
    ...
    for line in lcpn:
      ...
      if root != ancestor: # compute cumulative probabilites for each function in level
        for i, c in enumerate(children):
          cid = idx_children[i]
          pred[:,cid] = pred[:,cid] * pred[:,idx_ancestor] # compute cumprob for random forest
          # pred[:,cid] = np.minimum(pred[:,cid], pred[:,idx_ancestor]) # compute cumprob for random forest
  ...
  results.to_csv("predictions/lcpn.csv", index=False)
  ...
  results_pl.to_csv("predictions/lcpn_pl.csv", index=False)
  nlevels = len(lcl)
